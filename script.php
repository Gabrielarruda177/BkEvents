...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
    ...
    $consulta = $con->prepare("SELECT COUNT(*) FROM usuarios WHERE email = ?");
    ...
    if (move_uploaded_file($foto_temp, $caminho_foto)) {
        try {
            ...
            $comando = "INSERT INTO usuarios (nome, idade, email, senha, foto) VALUES (?, ?, ?, ?, ?)";
